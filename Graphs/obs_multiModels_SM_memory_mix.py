...
colocated_models = np.array([3, 4, 2, 3, 3, 3, 2, 2])

# dummy = [10.9,10.8,10.8,10.9]
# dummy2 = [2.2,2.3,2.2,2.2]

ax.bar(ind,colocated_models,width,color=color_list[2],hatch='x',edgecolor=color_list[2],fill=False,linewidth=2)
...
ax.set_ylabel('Number of spatially\nmultiplexed models')
ax.set_xlabel('GPUs')
...
xvalues = ["G1","G2","G3","G4","G5","G6","G7","G8"]
ax.set_xticklabels(xvalues, rotation = 45)

box = ax.get_position()
#box.height*0.75
#box.y0 + box.height * 0.32
ax.set_position([box.x0 + box.width*0.08, box.y0 + box.height*0.15, box.width, box.height*0.6])
