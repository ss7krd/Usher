...
proposed_igpr = (np.array([98.9, 75, 70, 79, 98.9]))
...
ax.bar(ind,proposed_igpr,width,color=color_list[0],hatch='+',edgecolor=color_list[0],fill=False,linewidth=2)
...
static_igpr = (np.array([98.9, 62, 68, 70, 98.9]))
...
ax.bar(ind+1*width, static_igpr, width, color = color_list[1], hatch='',edgecolor = color_list[1],fill=True,linewidth=2)
...
ax.set_position([box.x0 + box.width*0.1, box.y0 + box.height*0.3, box.width, box.height*0.55])
#bbox_to_anchor=(0.5, 1.6)
leg = plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.7), handler_map={matplotlib.lines.Line2D: SymHandler()}, 
            fontsize='60', ncol=1, handleheight=1, labelspacing=0.5, frameon=False) 
