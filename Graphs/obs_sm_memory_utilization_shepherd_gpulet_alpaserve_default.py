...
shepherd_computation = (np.array([96,95,81,45,97,94,41,38,81,42,97,68]))
gpulet_computation = (np.array([95,96,55,93,96,60,90,72,95,92,88,65]))
...
ax[0].bar(ind,shepherd_computation,width,color=color_list[0],hatch='+',edgecolor=color_list[0],fill=False,linewidth=2)
ax[1].bar(ind,gpulet_computation,width,color=color_list[0],hatch='+',edgecolor=color_list[0],fill=False,linewidth=2)
...
shepherd_memory = (np.array([55,52,65,97,72,69,67,65,53,65,51,70]))
gpulet_memory = (np.array([72,66,69,55,72,68,97,52,69,52,65,75]))
...
ax[0].bar(ind+1*width, shepherd_memory, width, color = color_list[1], hatch='',edgecolor = color_list[1],fill=True,linewidth=2)
ax[1].bar(ind+1*width, gpulet_memory, width, color = color_list[1], hatch='',edgecolor = color_list[1],fill=True,linewidth=2)
ax[2].bar(ind+1*width, alpaserve_memory, width, color = color_list[1], hatch='',edgecolor = color_list[1],fill=True,linewidth=2)

ax[0].set_title('Shepherd', fontsize=40)
ax[1].set_title('GPUlet', fontsize=40)
ax[2].set_title('AlpaServe', fontsize=40)

# ax[0].set_ylabel('Utilization')
fig.text(0.06,0.55, 'Utilization', va = 'center', rotation = "vertical", fontsize=40)
...
ax[2].set_xticklabels(xvalues, rotation = 45)
...
#box.height*0.75
#box.y0 + box.height * 0.32
ax[1].set_position([box.x0 + box.width*0.1, box.y0 + box.height*0.25, box.width, box.height*0.8])


box = ax[0].get_position()
#box.height*0.75
#box.y0 + box.height * 0.32
ax[0].set_position([box.x0 + box.width*0.1, box.y0 + box.height*0.20, box.width, box.height*0.8])
#bbox_to_anchor=(0.5, 1.6)
leg = ax[0].legend(loc='upper center', bbox_to_anchor=(0.45, 1.9), handler_map={matplotlib.lines.Line2D: SymHandler()}, 
            fontsize='40', ncol=2, handleheight=1, labelspacing=0.2, frameon=False) 
