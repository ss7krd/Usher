...
shepherd_computation_wo_holistic = (np.array([96,38,81,45,97,81,41,95,94,42,97]))+5
shepherd_computation_holistic = (np.array([95,74,96,78,82,80,78,80,75,78,5]))-5
...
ax[0].bar(ind,shepherd_computation_wo_holistic,width,color=color_list[0],hatch='+',edgecolor=color_list[0],fill=False,linewidth=2)
...
ax[0].bar(ind,dummy,width,color=color_list[0],edgecolor=color_list[0],hatch='+',fill=False,label="Computation\nutilization",linewidth=2)
# ax.bar(ind,dummy,width,color='lime',edgecolor='black',label="CSIR")
ax[0].bar(ind,dummy,width,color=color_list[1],edgecolor=color_list[1],hatch='/',fill=True,label="Memory\nutilization",linewidth=2)
...
ax[1].bar(ind+1*width, shepherd_memory_holistic, width, color = color_list[1], hatch='',edgecolor = color_list[1],fill=True,linewidth=2)
...
fig.text(0.01,0.55, 'Utilization', va = 'center', rotation = "vertical", fontsize=60)
...
ax[0].set_yticks([50, 100])
ax[1].set_yticks([50, 100])
