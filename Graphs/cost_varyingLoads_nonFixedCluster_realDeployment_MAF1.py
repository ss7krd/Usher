...
our_system = [73.44,122,200,240,336]#[336: 8 A100 + 3 V100, so heterogeneous better, also higher capacity GPU types preferred]
ax.plot(stepX,np.array(our_system)/divider,c=color_list[0],marker="x", mew=4, markersize=26,ls='-',label="Usher",fillstyle='none', linewidth = 4)
...
ax.plot(stepX[:-1], np.array(shepherd)/divider, c =color_list[1], marker = 's', mew=5, markersize = 26, ls = '-', label = "Shepherd", fillstyle = 'none', linewidth = 4)

gpulet = [230,250,275,350]
gpulet = np.array(gpulet)*2.2
# ax.plot(stepX, np.array(static)-5, c = 'blue', marker = 'o', markersize = 26, ls = '-', label = "Scrooge", fillstyle = 'full', linewidth = 4)
ax.plot(stepX[:-1], np.array(gpulet)/divider, c = color_list[2], marker = 'o', mew=4, markersize = 26, ls = '-', label = "GPUlet", fillstyle = 'none', linewidth = 4)

alpaserve = [170,220,250,300]
alpaserve = np.array(alpaserve)*2.2
# ax.plot(stepX, np.array(static)-5, c = 'blue', marker = 'o', markersize = 26, ls = '-', label = "Scrooge", fillstyle = 'full', linewidth = 4)
ax.plot(stepX[:-1], np.array(alpaserve)/divider, c = color_list[3], marker = 'D', mew=3, markersize = 26, ls = '-', label = "AlpaServe", fillstyle = 'none', linewidth = 4)
