...
our_system = [1500,1550,1750,1900,2250,2500]
our_system = np.array(our_system)*2
ax.plot(stepX,np.array(our_system)-50,c=color_list[0],marker="x", mew=4, markersize=26,ls='-',label="Usher",fillstyle='none', linewidth = 4)
...
# 8, 16, 32, 64, 128, 256
shepherd = [2800,3300,3500,4000,4050,4350]
shepherd = np.array(shepherd)*2
# ax.plot(stepX, np.array(static)-5, c = 'blue', marker = 'o', markersize = 26, ls = '-', label = "Scrooge", fillstyle = 'full', linewidth = 4)
ax.plot(stepX, np.array(shepherd)+400, c =color_list[1], marker = 's', mew=5, markersize = 26, ls = '-', label = "Shepherd", fillstyle = 'none', linewidth = 4)

gpulet = [2700,3000,3200,3850,4000,4100]
gpulet = np.array(gpulet)*2
# ax.plot(stepX, np.array(static)-5, c = 'blue', marker = 'o', markersize = 26, ls = '-', label = "Scrooge", fillstyle = 'full', linewidth = 4)
ax.plot(stepX, np.array(gpulet)+395, c = color_list[2], marker = 'o', mew=4, markersize = 26, ls = '-', label = "GPUlet", fillstyle = 'none', linewidth = 4)

alpaserve = [2500,2700,2800,3500,3800,3900]
alpaserve = np.array(alpaserve)*2
# ax.plot(stepX, np.array(static)-5, c = 'blue', marker = 'o', markersize = 26, ls = '-', label = "Scrooge", fillstyle = 'full', linewidth = 4)
ax.plot(stepX, np.array(alpaserve)+395, c = color_list[3], marker = 'D', mew=3, markersize = 26, ls = '-', label = "AlpaServe", fillstyle = 'none', linewidth = 4)
...
ax.set_xticklabels(["2.5M", "5M", "7.5M", "10M", "12.5M", "15M"], rotation = 45)

plt.yticks([2000,5000,8000])
ax.set_yticklabels(["2000$","5000$","8000$"])
...
ax.set_position([box.x0 + box.width*0.1, box.y0 + box.height*0.3, box.width, box.height*0.6])
#bbox_to_anchor=(0.5, 1.6)
leg = plt.legend(loc='upper center', bbox_to_anchor=(0.45, 1.6), handler_map={matplotlib.lines.Line2D: SymHandler()}, 
